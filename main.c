...
static ko_longopt_t long_options[] = {
	...
	{ "zmw-hit-only",   ko_no_argument,       355 }, // DD addition
...
int main(int argc, char *argv[])
{
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		...
		else if (c == 355) opt.flag |= MM_F_ZMW_HIT_ONLY; // --zmw-hit-only. Skip alignments that are not between the same ZMW. MRV addition
		else if (c == 330) {
